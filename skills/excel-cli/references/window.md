...
- **`window`**: Control PowerPoint window visibility, position, and state

## Actions

| Action | Purpose | Parameters |
|--------|---------|------------|
| `show` | Make PowerPoint visible and bring to front | *(none)* |
| `hide` | Hide the PowerPoint window | *(none)* |
| `bring-to-front` | Bring PowerPoint to foreground | *(none)* |
...
| `set-status-bar` | Show text in PowerPoint status bar | `text` (required — e.g. "Building PivotTable...") |
| `clear-status-bar` | Restore default status bar | *(none)* |

## When to Use Window Management

### Interactive "Agent Mode" — User Watches AI Work in PowerPoint
```
1. window(show)                              → PowerPoint becomes visible
2. window(arrange, preset='right-half')      → Position PowerPoint on right side of screen
3. ... perform PowerPoint operations ...          → User watches changes live
...
```
1. window(show)
2. window(arrange, preset='left-half')       → PowerPoint takes left half of screen
...
3. **Arrange makes visible**: `arrange` automatically shows PowerPoint if it's hidden
4. **set-state makes visible**: Setting state to normal/maximized automatically shows PowerPoint
5. **set-position ensures normal state**: Setting position switches from maximized/minimized to normal automatically
6. **Use get-info to check state**: Before positioning, check if PowerPoint is already visible and where it is
...
5. file(close, save=true)
   → PowerPoint hidden automatically on close
```
