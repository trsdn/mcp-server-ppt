...
PowerPoint Tables on slides are NOT automatically in the Data Model (Power Pivot).
To analyze slide data with DAX measures:

1. Ensure data is formatted as a PowerPoint Table (use create action if needed)
...
- add-to-data-model: Add an existing slide table to Power Pivot for DAX analysis
...
PowerPoint Tables manage their own header/row/totals formatting through table styles. Never use `range_format(action: 'format-range')` on table header rows — it conflicts with the table style and produces inconsistent formatting.
...
Create slide tables populated by DAX EVALUATE queries against the Data Model.
...
- Only works on PowerPoint Tables (ListObjects), not plain ranges
...
| Goal | Tool |
|------|------|
| Create/manage slide tables | table |
| Add slide table to Power Pivot | table (add-to-data-model) |
...
- Table names must be unique within presentation (PowerPoint requirement)
