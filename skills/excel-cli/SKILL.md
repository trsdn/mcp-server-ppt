...
**Actions:** `create`, `get-info`, `set-title`, `set-type`, `delete`
...
| `--shape-name` | (required for: get-info, set-title, set-type, delete) |
...
**Actions:** `to-pdf`, `slide-to-image`, `to-video`, `print`, `save-as`
...
| `--width` | Image width in pixels (default: 1920) (required for: slide-to-image) |
| `--height` | Image height in pixels (default: 1080) (required for: slide-to-image) |
...
**Actions:** `list`, `set-text`

| Parameter | Description |
|-----------|-------------|
| `--slide-index` | 1-based slide index (required) |
| `--placeholder-index` | 1-based placeholder index (required for: set-text) |
...
**Actions:** `list`, `read`, `create`, `duplicate`, `move`, `delete`, `apply-layout`, `set-name`

| Parameter | Description |
|-----------|-------------|
| `--slide-index` | 1-based slide index (required for: read, duplicate, move, delete, apply-layout, set-name) |
...
**Actions:** `create`, `read`, `write-cell`, `add-row`, `add-column`, `delete-row`, `delete-column`, `merge-cells`, `format-cell`
...
| `--width` | Width in points (required for: create) |
| `--height` | Height in points (required for: create) |
| `--shape-name` | Name of the table shape (required for: read, write-cell, add-row, add-column, delete-row, delete-column, merge-cells, format-cell) |
| `--row` | 1-based row index (required for: write-cell, delete-row, format-cell) |
| `--column` | 1-based column index (required for: write-cell, delete-column, format-cell) |
...
**Actions:** `get`, `set`, `find`, `replace`, `format`, `format-advanced`
