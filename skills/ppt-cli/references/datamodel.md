...
After installation, restart PowerPoint and try again.

## CRITICAL: Data Model Sync (Slide Tables)

**Slide tables and Data Model tables are SEPARATE copies!**

When you append/modify a slide table, the Data Model does NOT auto-update.
...
- After `table(append)` to slide table
- After `range(set-values)` that modifies table data
- After any manual/direct slide edits

## PowerPoint Power Pivot Limitations (vs SSAS/Power BI)

| Feature | Power BI/SSAS | PowerPoint Power Pivot | Workaround |
|---------|---------------|-------------------|------------|
...
**Key Insight**: PowerPoint's COM API cannot create or modify calculated columns. If you need computed columns:
...
| Source | Method |
|--------|--------|
| Slide PowerPoint Table | table with add-to-data-model action |
...
DAX formulas are automatically formatted on WRITE operations only (create-measure, update-measure) using the official Dax.Formatter library (SQLBI). Read operations (list-measures, read) return raw DAX as stored in PowerPoint. Formatting adds ~100-500ms network latency per write operation but ensures consistent, professional code formatting. If formatting fails (network issues, API errors), the original DAX is saved unchanged - operations never fail due to formatting.

**Action disambiguation**:

- list-tables: List all tables currently in the Data Model
- list-measures: List all DAX measures (returns raw DAX from PowerPoint)
...
IMPORTANT LIMITATIONS (PowerPoint's embedded Analysis Services):
...
- PowerPoint's embedded AS has limited support compared to full SQL Server Analysis Services

**Working DMV queries (verified in PowerPoint):**
...
**DMV queries that execute but may return empty in PowerPoint:**

| DMV Query | Notes |
|-----------|-------|
| `SELECT * FROM $SYSTEM.TMSCHEMA_TABLES` | May return 0 rows in PowerPoint's embedded AS |
| `SELECT * FROM $SYSTEM.TMSCHEMA_COLUMNS` | May return 0 rows in PowerPoint's embedded AS |
| `SELECT * FROM $SYSTEM.TMSCHEMA_PARTITIONS` | May return 0 rows in PowerPoint's embedded AS |
...
- Using slide table names instead of Data Model table names
...
- Table names in Data Model may differ from slide (check list-tables)
