...
description: >
  ...
  or modifying PowerPoint presentations. Supports Power Query (M code), Data Model (DAX measures),
  PivotTables, Tables, Ranges, Charts, Slicers, Formatting, VBA macros, connections, and calculation mode control.
  Triggers: PowerPoint, presentation, pptx, Power Query, DAX, PivotTable, VBA.
---
...
3. calculation_mode(action: 'calculate', scope: 'presentation')  → Recalculate once
...
### Rule 4: Use PowerPoint Tables (Not Plain Ranges)

Always convert tabular data to PowerPoint Tables:
...
**Common mistake:** Creating/updating without evaluate → pollutes presentation with broken queries
...
3. calculation_mode(action: 'calculate', scope: 'presentation')  → Recalculate once at end
4. calculation_mode(action: 'set-mode', mode: 'automatic')  → Restore default
```
