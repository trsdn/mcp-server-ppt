# DMV Query Reference (PowerPoint's Embedded Analysis Services)

SYNTAX: `SELECT * FROM $SYSTEM.<SchemaRowset>`

LIMITATIONS:
- ONLY `SELECT *` works — specific column selection fails
- Some TMSCHEMA views return empty results in PowerPoint's embedded AS
...
## May Return Empty in PowerPoint
