...
|--------|----------------------|------------|
| PowerPoint Tables | `table(action:'set-style')` or `tableStyle` on create | `range_format` on header/data rows |
...
AI: "Would you like me to create a new PowerPoint file for the sales report?"
User: "Yes"
AI: "What would you like to name the file?"
User: "sales_report.pptx"
...
User: "Create a sales report"
AI: "Created sales report at C:\Users\You\Documents\sales_report.pptx with the following structure:
...
file(action: 'open', filePath: 'file1.pptx')  // Session 1
file(action: 'open', filePath: 'file2.pptx')  // Session 2
file(action: 'open', filePath: 'file3.pptx')  // Session 3
// ... never closed
```

Results:
- PowerPoint processes accumulate
- Memory usage grows
...
session1 = file(action: 'open', path: 'file1.pptx')
// ... work with file1 ...
file(action: 'close', sessionId: session1, save: true)

session2 = file(action: 'open', path: 'file2.pptx')
// ... work with file2 ...
...
Always use US format codes (PowerPoint translates automatically):
...
PowerPoint displays the result in the user's locale setting, but the API requires US format input.
...
// Now presentation is polluted with broken query
```

This causes:
- Broken queries persisted in presentation
- Cryptic COM exceptions instead of helpful M error messages
- Need manual PowerPoint cleanup to remove broken queries
...
- Copying known-working query from another presentation
