...
**Test BEFORE persisting - avoid polluting presentations with broken queries:**

```
Step 1: evaluate → Test M code, verify results (catches syntax errors, missing sources)
Step 2: create/update → Store VALIDATED query in presentation
...
- `worksheet` (default): Creates a PowerPoint Table on a slide
- `data-model`: Loads directly to Power Pivot for DAX analysis
- `both`: Loads to slide AND Power Pivot
...
Alternative path (for existing slide tables):
...
- load-to: Loads to slide or data model or both (not just config change) - CHECKS for sheet conflicts
- unload: Removes data from ALL destinations (slide AND Data Model) - keeps query definition
- delete: Completely removes query AND all associated data (slide, Data Model connections)
...
- No auto-save: presentation must be saved separately to persist the rename
...
- `IsConnectionOnly=true` means query has NO data destination (not in slide, not in Data Model)
- `IsConnectionOnly=false` means query loads data SOMEWHERE (slide OR Data Model OR both)
...
- Use `targetCellAddress` to place the table on an existing slide without deleting other content
- Applies to BOTH create and load-to
- If the slide already has data and you omit `targetCellAddress`, the tool returns guidance telling you to provide one
- Existing tables are refreshed in-place; specifying a different `targetCellAddress` requires unload + reload
- Slides that exist but are empty behave like new sheets (default destination = A1)

**Common mistakes**:

- **WARNING: Skipping evaluate** → Create/update with untested M code (ERROR: pollutes presentation with broken queries)
...
- Assuming unload only removes slide data → Also removes Data Model connections
...
- load-to uses the same 30-minute timeout as refresh. If PowerPoint is blocked by privacy dialogs/credentials, you'll get `SuggestedNextActions` instead of a hang—surface them to the user before retrying.

**Data Model connection cleanup**:

- Unload removes BOTH slide ListObjects AND Data Model connections
- Delete removes query, slide ListObjects, AND Data Model connections
...
Query naming: File name MUST match PowerPoint query name exactly.
