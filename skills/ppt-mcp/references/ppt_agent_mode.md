...
PowerPoint MCP's Agent Mode lets users watch AI operations happen in real-time. Instead of hidden automation, users see PowerPoint respond to commands live — like Microsoft's Agent Mode in PowerPoint, but with the full power of 225 COM-level operations.

> **MCP Server feature only.** Agent Mode uses conversational UI to ask about visibility preferences.

## When to Offer Agent Mode

**Always ask the user** at session start whether they want PowerPoint visible or hidden. Present two clear choices using action cards:
...
3. window(arrange, preset='right-half')            → PowerPoint on right, AI on left
...
AI navigates through a completed presentation, explaining findings while the user watches.
...
Use `window(set-status-bar)` to show operation progress in PowerPoint's status bar:

| Operation | Status Bar Text |
|-----------|----------------|
| Writing data | `"PptMcp: Writing 500 rows to Sales sheet..."` |
| Building PivotTable | `"PptMcp: Building PivotTable from Sales data..."` |
| Refreshing query | `"PptMcp: Refreshing Power Query 'Revenue'..."` |
| Creating chart | `"PptMcp: Creating bar chart from sales data..."` |
| Formatting | `"PptMcp: Applying currency formatting..."` |

**Always clear** with `window(clear-status-bar)` when the workflow completes.

**Only set when visible**: Status bar text is only useful when PowerPoint is visible. Skip status bar calls when PowerPoint is hidden.
...
1. `window(show)` → Make PowerPoint visible
...
If the user picks "Work in background" or doesn't respond, keep PowerPoint hidden and skip all status bar calls.
