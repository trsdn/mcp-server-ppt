...
- `borderStyle`: PowerPoint border style name
...
pptcli conditionalformat add-rule --session <id> --sheet-name "Data" --range-address "B2:B100" `
...
pptcli conditionalformat add-rule --session <id> --sheet-name "Data" --range-address "A2:E100" `
  --rule-type "expression" --formula1 "=`$A2=`"Error`"" --interior-color "#FF0000" --font-color "#FFFFFF"

# Clear all rules from range
pptcli conditionalformat clear-rules --session <id> --sheet-name "Data" --range-address "A1:E100"
```
...
1. Test expression formulas in PowerPoint first to verify logic
