...
using PptMcp.Generators.Common;
...
public class ServiceRegistryGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(Compilation compilation, ImmutableArray<InterfaceDeclarationSyntax?> interfaces, SourceProductionContext context)
    {
        ...
        // Category + fully-qualified interface name, used to generate the dispatch table.
        var dispatchTableEntries = new List<(ServiceInfo Info, string InterfaceFullName)>();
        ...
        foreach (var interfaceDecl in distinctInterfaces)
        {
            ...
            dispatchTableEntries.Add((info, interfaceFullName));
        ...
        if (allCategories.Count > 0)
        {
            ...
            // Emit the category dispatch table so the service cannot drift from the
            // generated tool surface (GitHub #124).
            var dispatchTableCode = GenerateServiceDispatchTable(dispatchTableEntries);
            context.AddSource("ServiceRegistry.DispatchTable.g.cs", SourceText.From(dispatchTableCode, Encoding.UTF8));

            // Emit parameter XML docs as constants. This compilation is the only one that
            // can read them, because the downstream generators see Core as a metadata
            // reference and the compiler does not load reference XML docs (GitHub #128).
            var parameterDocsCode = GenerateParameterDocs(allCategories);
            context.AddSource("_ParameterDocs.g.cs", SourceText.From(parameterDocsCode, Encoding.UTF8));
        }
    }
    ...
    private static string GenerateServiceRegistry(ServiceInfo info)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("namespace PptMcp.Generated;");
        sb.AppendLine();
    ...
    private static void GenerateCliRouteMethod(StringBuilder sb, ServiceInfo info)
    {
        ...
        foreach (var method in info.Methods)
        {
            ...
            foreach (var p in method.Parameters.Where(p => !p.IsFileOrValue && (p.IsRequired || (!p.HasDefault && !p.TypeName.EndsWith("?"))) && StringHelper.IsStringType(p.TypeName)))
            {
                ...
                sb.AppendLine($"                    PptMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty({paramName}, \"{paramName}\", \"{method.ActionName}\");");
            }
            ...
            {
                sb.AppendLine($"                    var resolved{StringHelper.ToPascalCase(p.Name)} = PptMcp.Core.Utilities.ParameterTransforms.ResolveFileOrValue({p.Name}, {p.Name}{p.FileSuffix});");
                // Validate required FileOrValue parameters AFTER resolution
                if (p.IsRequired || (!p.HasDefault && !p.TypeName.EndsWith("?")))
                {
                    sb.AppendLine($"                    PptMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty(resolved{StringHelper.ToPascalCase(p.Name)}, \"{p.Name}\", \"{method.ActionName}\");");
                }
            }
    ...
    /// <summary>
    /// Emits every parameter's <c>&lt;param&gt;</c> XML doc as a <c>const</c> string.
    ///
    /// This compilation is the only place the docs are readable: the MCP and CLI
    /// generators run in their own projects, where Core is a metadata reference and
    /// <c>GetDocumentationCommentXml()</c> returns nothing because the compiler does not
    /// load XML documentation for references. Constants survive into metadata, so they
    /// travel where the XML docs cannot (GitHub #128).
    /// </summary>
    private static string GenerateParameterDocs(List<ServiceInfo> categories)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Parameter XML documentation captured from the Core interfaces.");
        sb.AppendLine("/// Read back by the MCP and CLI generators, which cannot see Core's XML docs.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public static class _ParameterDocs");
        sb.AppendLine("{");
        ...
        {
            foreach (var param in ServiceInfoExtractor.GetAllExposedParameters(cat))
            {
                if (string.IsNullOrWhiteSpace(param.Description))
                    continue;

                var key = ParameterDocLookup.Key(cat.CategoryPascal, param.Name);
                if (!emitted.Add(key))
                    continue;

                sb.AppendLine($"    public const string {key} = {ToLiteral(param.Description!)};");
            }
        }
    ...
    /// <summary>Escapes a string as a C# verbatim-safe literal.</summary>
    ...
    private static string GenerateSkillManifest(List<ServiceInfo> categories)
    {
        ...
        sb.AppendLine("/// Used by PptMcp.Build.Tasks to generate SKILL.md files from templates.");
        sb.AppendLine("/// </summary>");
    ...
    // PptMcp.Generators.Shared and included as source files
    ...
    private static string GenerateDispatchHelpers()
    {
        ...
        sb.AppendLine("        //   echo '[[\"value\",1]]' | pptcli range set-values --values -");
    ...
    /// <summary>
    /// Generates the category dispatch table consumed by PptMcpService.
    ///
    /// The MCP tool surface and per-category action dispatch are both generated from the
    /// Core interfaces, but the service's top-level category switch used to be hand-written.
    /// The two drifted and 11 advertised categories became unroutable (GitHub #124).
    /// Generating the table from the same interfaces makes that drift impossible.
    /// </summary>
    private static string GenerateServiceDispatchTable(List<(ServiceInfo Info, string InterfaceFullName)> entries)
    {
        var sb = new StringBuilder();
        ...
        // Delegate used by every generated category entry.
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Dispatches an action within a category. Returns false when the action is unknown,");
        sb.AppendLine("    /// so callers can distinguish an invalid action from a void operation returning null.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    public delegate bool CategoryDispatch(");
        sb.AppendLine("        object commands,");
        sb.AppendLine("        string actionString,");
        sb.AppendLine("        PptMcp.ComInterop.Session.IPptBatch? batch,");
        sb.AppendLine("        string? argsJson,");
        sb.AppendLine("        out string? result);");
        sb.AppendLine();

        sb.AppendLine("    /// <summary>Metadata and dispatch entry point for a single service category.</summary>");
        sb.AppendLine("    public sealed class CategoryEntry");
        sb.AppendLine("    {");
        sb.AppendLine("        public CategoryEntry(");
        sb.AppendLine("            string category,");
        sb.AppendLine("            bool requiresSession,");
        sb.AppendLine("            System.Func<object> createCommands,");
        sb.AppendLine("            System.Func<string, bool> isValidAction,");
        sb.AppendLine("            CategoryDispatch dispatch)");
        sb.AppendLine("        {");
        sb.AppendLine("            Category = category;");
        sb.AppendLine("            RequiresSession = requiresSession;");
        sb.AppendLine("            CreateCommands = createCommands;");
        sb.AppendLine("            IsValidAction = isValidAction;");
        sb.AppendLine("            Dispatch = dispatch;");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        /// <summary>Wire-level category name, e.g. \"comment\".</summary>");
        sb.AppendLine("        public string Category { get; }");
        sb.AppendLine();
        sb.AppendLine("        /// <summary>False for [NoSession] categories such as file.</summary>");
        sb.AppendLine("        public bool RequiresSession { get; }");
        sb.AppendLine();
        sb.AppendLine("        /// <summary>Creates the Core commands instance backing this category.</summary>");
        sb.AppendLine("        public System.Func<object> CreateCommands { get; }");
        sb.AppendLine();
        sb.AppendLine("        /// <summary>Validates an action string before a session is acquired.</summary>");
        sb.AppendLine("        public System.Func<string, bool> IsValidAction { get; }");
        sb.AppendLine();
        sb.AppendLine("        /// <summary>Routes a parsed action to the Core command method.</summary>");
        sb.AppendLine("        public CategoryDispatch Dispatch { get; }");
        sb.AppendLine("    }");
        sb.AppendLine();

        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Every service category, keyed by wire-level category name.");
        sb.AppendLine("    /// Generated from the [ServiceCategory] interfaces, so this is always complete.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    public static readonly System.Collections.Generic.IReadOnlyDictionary<string, CategoryEntry> CategoryTable =");
        sb.AppendLine("        new System.Collections.Generic.Dictionary<string, CategoryEntry>(System.StringComparer.Ordinal)");
        sb.AppendLine("        {");

        foreach (var (info, interfaceFullName) in entries.OrderBy(e => e.Info.Category, StringComparer.Ordinal))
        {
            var implFullName = ToImplementationTypeName(interfaceFullName);
            var pascal = info.CategoryPascal;

            sb.AppendLine($"            [\"{info.Category}\"] = new CategoryEntry(");
            sb.AppendLine($"                \"{info.Category}\",");
            sb.AppendLine($"                {(info.NoSession ? "false" : "true")},");
            sb.AppendLine($"                static () => new {implFullName}(),");
            sb.AppendLine($"                static a => {pascal}.TryParseAction(a, out _),");
            sb.AppendLine("                static (object commands, string actionString, PptMcp.ComInterop.Session.IPptBatch? batch, string? argsJson, out string? result) =>");
            sb.AppendLine("                {");
            sb.AppendLine("                    result = null;");
            sb.AppendLine($"                    if (!{pascal}.TryParseAction(actionString, out var action))");
            sb.AppendLine("                        return false;");
            sb.AppendLine();

            if (info.NoSession)
            {
                sb.AppendLine($"                    result = {pascal}.DispatchToCore(({interfaceFullName})commands, action, argsJson);");
            }
            else
            {
                sb.AppendLine("                    if (batch is null)");
                sb.AppendLine($"                        throw new System.InvalidOperationException(\"Category '{info.Category}' requires a session batch.\");");
                sb.AppendLine();
                sb.AppendLine($"                    result = {pascal}.DispatchToCore(({interfaceFullName})commands, action, batch, argsJson);");
            }
    ...
    /// <summary>
    /// Maps a Core commands interface to its concrete implementation by convention
    /// (I&lt;Name&gt;Commands -&gt; &lt;Name&gt;Commands in the same namespace).
    /// A convention break produces a compile error in the generated table rather than a
    /// silent routing gap.
    /// </summary>
    private static string ToImplementationTypeName(string interfaceFullName)
    {
        var lastDot = interfaceFullName.LastIndexOf('.');
        if (lastDot < 0)
        {
            return interfaceFullName.StartsWith("I", StringComparison.Ordinal)
                ? interfaceFullName.Substring(1)
                : interfaceFullName;
        }

        var ns = interfaceFullName.Substring(0, lastDot);
        var typeName = interfaceFullName.Substring(lastDot + 1);
    ...
    /// replacing hand-written Handle*CommandAsync methods in PptMcpService.
    /// </summary>
    ...
    private static void GenerateDispatchCase(StringBuilder sb, ServiceInfo info, MethodInfo method)
    {
        ...
        if (method.HasProgressParameter)
            callArgs.Add("PptMcp.ComInterop.ProgressContext.Current");
