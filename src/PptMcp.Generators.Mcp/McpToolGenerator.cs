...
namespace PptMcp.Generators.Mcp;
...
public class McpToolGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateToolClass(ServiceInfo info)
    {
        ...
        if (hasProgress)
        {
            ...
            sb.AppendLine("using PptMcp.ComInterop;");
            sb.AppendLine("using PptMcp.McpServer.Progress;");
        }
        sb.AppendLine();
        sb.AppendLine("namespace PptMcp.McpServer.Tools;");
        sb.AppendLine();
        ...
        foreach (var ep in exposedParams)
        {
                     ...
                     ep.TypeName.StartsWith("List<string>"))
            {
                ...
                result.Add(new McpParameter(
                    ...
                    defaultExpression: "null",
                    preProcessingCode: $"var {localVarName} = PptMcp.Core.Utilities.ParameterTransforms.ParseJsonList({snakeName}, nameof({snakeName}));"));
            }
            else
            {
