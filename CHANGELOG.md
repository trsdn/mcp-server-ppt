...
- **The v1.0.3 release reported four successful publishes but only reached NuGet**: the VS Code Marketplace, both npm skill packages and the MCP Registry never received the release, yet every step showed as successful in the job summary.
  - ROOT CAUSE: each of those steps carried `continue-on-error: true`, so a failed publish was indistinguishable from a successful one. Verified against the registries: `trsdn.ppt-mcp` is absent from the Marketplace (the `trsdn` publisher does not exist), `ppt-mcp-skill` and `ppt-cli-skill` are absent from npm, and the server is absent from the MCP Registry, while both NuGet packages are present
  - FIX: removed `continue-on-error` from the npm and MCP Registry publishes and added a post-publish npm listing check. The Marketplace publish is now opt-in through a `publish_vscode` workflow input and is verified against the Marketplace API, so a silent failure fails the job
- **Documentation advertised a VS Code Marketplace listing that returns 404**: seven places, including the main README and the installation guide, linked to `itemName=trsdn.ppt-mcp` as a one-click install. They now point to the `.vsix` in the GitHub release, which is the artifact that actually exists.
